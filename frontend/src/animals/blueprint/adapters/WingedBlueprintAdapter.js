/**
 * WingedBlueprintAdapter
 * ----------------------
 *
 * Converts a SpeciesBlueprint with a winged body plan into a canonical,
 * deterministic representation with required wing/leg chains. Mirrors the
 * quadruped adapter flow to keep validation and runtime payloads consistent
 * with the Zoo-style pipeline.
 */

const REQUIRED_CHAINS = ["spine", "neck", "head", "wingLeft", "wingRight", "legL", "legR"];
...
export class WingedBlueprintAdapter {
  /**
   * Adapt a winged SpeciesBlueprint into a canonical compiled representation.
   *
  ...
  adapt(blueprint) {
    if (!blueprint) {
      throw new Error("WingedBlueprintAdapter requires a SpeciesBlueprint");
    }
  ...
  #validateBodyPlan(blueprint, validation) {
    ...
    if (type !== "winged") {
      validation.errors.push(`Expected bodyPlan.type=\"winged\" but received ${type || "undefined"}`);
    }
  }
  ...
  #buildRuntimeBlueprint(blueprint, canonicalSkeleton, canonicalChains, canonicalBodyParts) {
    ...
    runtimeBlueprint.bodyPlan = { ...(runtimeBlueprint.bodyPlan || {}), type: "winged" };
...
export default WingedBlueprintAdapter;
